...
N=1001; % Vector size for simulations
...
%         Transducer(floor(N/2)+Number_of_Elements/2:end) = 0; % set to zero the pixels outside the transducer
        ...
        RMS(n) = sum(abs(desired_Output_Shift).^2-abs(Output).^2);
        if Show_GS_Process == 1
            bar(x*1e3,abs(Output).^2); title(['Output Abs n=',num2str(n)]); xlim([-10,10]); xlabel('x[mm]'); ylim([0,1])
            pause(0.5)
        end
    end
...
Transducer_2pix = Transducer(250:750);
