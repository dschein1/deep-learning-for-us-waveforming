...
P.numRays = 512;   % 128 no. of Rays (1 for Flash transmit)
startDepth = 10; %[mm]
endDepth = 20; %[mm]
Trans.frequency = 3.0488; %[MHz]
% Trans.frequency = 3.4722; %[MHz]
% Trans.frequency = 4.4643; %[MHz]
% Trans.frequency = 5.4348; %[MHz]
...
Focus = 31.5; %[mm]
P.txFocus = (Focus*1e-3)/Resource.Parameters.speedOfSound*Trans.frequency*1e6; % initial value of P.txFocus was 100
...
Resource.Parameters.Connector = 1 ; % the values 1 and 2 select the left or right connector, respectively (if not provided, defaults to 1)
...
PData(1).PDelta = [0.00875, 0, 0.05];  % x, y, z pdeltas
PData(1).Size(1) = 10 + ceil((P.endDepth-P.startDepth)/PData(1).PDelta(3));
PData(1).Size(2) = 10 + ceil((P.endDepth-P.startDepth)/PData(1).PDelta(1));
...
Resource.DisplayWindow(1).pdelta = 0.2; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 0.35 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
