...
clear all
cd 'C:\Users\Administrator\Documents\Vantage-4.1.1-1910101200'
...
P.endDepth = (endDepth*1e-3)/Resource.Parameters.speedOfSound*Trans.frequency*1e6;   % 235 = 80mm Acquisition depth in wavelengths
...
P.radius = 0; % placing the origin at the transducer center
...
Resource.RcvBuffer(1).rowsPerFrame = 2048*P.numRays; % originally was 2048*P.numRays (This is for the max range on range slider.) 
...
path = 'C:\Users\Administrator\Documents\MATLAB\Raphael\excitation matrices\TW\';
file = 'TW_focus_[20,30,40,50,60]_TF_[4.4643]_WF_[4.4643,4.4643,4.4643,4.4643,4.4643].mat';
ArbWav = load([path file]);
TW = ArbWav.TW;

% path = 'C:\Users\Administrator\Documents\MATLAB\Raphael\20210425\Organised - Exp 20171121 - Wire target in water - EXP4\';
% file = 'ArbWav_60_40_20.mat';
% ArbWav = load([path file]);
% TW = ArbWav.a;
...
% BPF2 = [+0.00046 +0.00000 -0.00351 +0.00000 +0.00000 +0.00000 +0.01297 ...
%  +0.00000 +0.00000 +0.00000 -0.03491 +0.00000 +0.00000 +0.00000 ...
%  +0.08612 +0.00000 +0.00000 +0.00000 -0.31116 +0.00000 +0.50006]; % center freq 5.9524 MHz - BW relative=1
% % BPF2 = ones(1,21);
Receive = repmat(struct('Apod', ones(1,Trans.numelements), ...
                        ...
                        'framenum', 1, ...
                        'acqNum', 1, ...
                        'sampleMode', 'BS100BW', ... %NS200BW/NS200BW      % 'InputFilter', BPF2, ... 
                        'mode', 0, ...
...
ReconInfo = repmat(struct('mode', 'replaceIntensity', ...  % replace data. %replaceIntensity 
                   'txnum', 1, ...
                   'rcvnum', 1, ...
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    if floor(i/3) == i/3     % Exit to Matlab every 4th frame reconstructed 
...
save('MatFiles/P6_3_ArbWav');
filename = 'P6_3_ArbWav';
VSX
return
...
IQfilename = ['C:\Users\Administrator\Documents\MATLAB\Raphael\20210425\IQdataArbWav-focus_[]_TF_[]_WF_[]'];
